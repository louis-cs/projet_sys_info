...
%type <entier> tNB Exp If tIF tELSE tWHILE tPARO
%type <str> tID

/* Keywords */
%token tCONST tINT tMAIN tPRINTF tIF tELSE tWHILE
/* Operators */
%token tMUL tPLUS tMINUS tDIV tEQU tCOMP tINF tSUP tINFE tSUPE
/* Delimiters */
...
Instruction           : Affectation | Print | IfElse | While;
...
Exp                   : Exp tPLUS Exp {ins_add(tins,LOAD,0,get_addr(ts,$1),-1);
                      ...
                      | Exp tCOMP Exp {ins_add(tins,LOAD,0,get_addr(ts,$1),-1);
                                       ins_add(tins,LOAD,1,get_addr(ts,$3),-1);
                      ...
                      | Exp tINF Exp {ins_add(tins,LOAD,0,get_addr(ts,$1),-1);
                                      ins_add(tins,LOAD,1,get_addr(ts,$3),-1);
                      ...
                      | Exp tSUP Exp {ins_add(tins,LOAD,0,get_addr(ts,$1),-1);
                                      ins_add(tins,LOAD,1,get_addr(ts,$3),-1);
                      ...
                      | Exp tINFE Exp {ins_add(tins,LOAD,0,get_addr(ts,$1),-1);
                                       ins_add(tins,LOAD,1,get_addr(ts,$3),-1);
                      ...
                      | Exp tSUPE Exp {ins_add(tins,LOAD,0,get_addr(ts,$1),-1);
                                       ins_add(tins,LOAD,1,get_addr(ts,$3),-1);
...
IfElse                : If Body %prec tNO_ELSE {ins_update_Ri(tins, $1, ins_get_next_index(tins));}
                      | If Body tELSE {
                        int index = ins_add(tins, JMP, 0xFFFF, -1, -1);
                        ins_update_Ri(tins, $1, ins_get_next_index(tins));
                        $3 = index;
                        } Body {
